...
public class Jigsaw {
	...
	public boolean BFSearch() throws IOException {
        ...
        isCompleted = (currentJNode.equals(endJNode));

        while (openList.size() > 0 && !isCompleted) {
            int[] movable = currentJNode.canMove();            
            Vector<JigsawNode> followJNodes = findFollowJNodes(currentJNode);
            for (int i = 0; i < followJNodes.size(); ++i) {
                openList.add(followJNodes.get(i));
            }
